...
# For cooldown handling
last_visual_alert_time = 0
VISUAL_ALERT_COOLDOWN = 300  # 5 minutes
VISUAL_STD_THRESHOLD = 45    # Tune this empirically
VISUAL_FFT_THRESHOLD = 1.5   # Tune this empirically
...
def send_email(subject, body, attachment_path=None, theme="alert", stream_url=None):
    ...
    try:
        ...
        # Current timestamp
        timestamp_html = datetime.now().strftime("%A, %d %B %Y, %I:%M %p")

        # Construct HTML email body with header/footer
        html_body = f"""
        <html>
            <body style="font-family: Arial, sans-serif; background-color: #f5f5f5; padding: 20px;">
                <div style="max-width: 640px; margin: auto; background-color: white; border: 1px solid #ddd; border-radius: 8px; overflow: hidden;">
                    ...
                    <div style="background-color: #f0f0f0; padding: 15px; text-align: center; font-size: 13px; color: #666;">
                        <p style="margin: 4px;">This is an automated monitoring alert generated by the Stream Watchdog System.</p>
                        <p style="margin: 4px;">&copy; {datetime.now().year} <strong>Mercurial Minds Pvt Ltd</strong>. All rights reserved.</p>
                        <p style="margin: 4px;">Sent on {timestamp_html}</p>
                    </div>
                </div>
            </body>
        </html>
...
def setup_browser(tester_url, stream_url):
    ...
    # Use Service if available, else pass path directly
    if USE_SERVICE:
...
def main(tester_url, stream_url, stop_event):
    ...
    def video_thread():
        ...
        visual_alert_last_sent = 0  # Cooldown for visual artifact alerts
        ...
        while not stop_event.is_set():
            ...
            def alert_function():
                log_freeze_event("[ALERT] Visual artifact detected!", excel_file)
                glitch_image_path = "visual_glitch.jpg"
                cv2.imwrite(glitch_image_path, full_res_frame, [int(cv2.IMWRITE_JPEG_QUALITY), 80])
                threading.Thread(target=play_alert, daemon=True).start()
                send_email(
                    "⚠️ Visual Glitch Detected",
                    f"A visual artifact/glitch was detected in the stream.\n\n📺 Stream URL: {stream_url}",
                    glitch_image_path
                )

            def capture_screenshot_func(tag="visual_artifact"):
                timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                filename = f"{tag}_{timestamp}.jpg"
                cv2.imwrite(filename, full_res_frame, [int(cv2.IMWRITE_JPEG_QUALITY), 80])
